...
class _AdminScreenState extends State<AdminScreen> with SingleTickerProviderStateMixin {
  ...
  final List<String> categories = ['Men', 'Women', 'Packages', 'Kids', 'Hair', 'Body'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Perfume Admin", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
        centerTitle: true,
        bottom: TabBar(
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              ...
              child: ListTile(
                title: Text(data['sender_email'] ?? 'Anonymous', style: TextStyle(fontWeight: FontWeight.bold, color: gold)),
                subtitle: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Text(date != null ? DateFormat('yyyy-MM-dd hh:mm a').format(date) : '', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                  ],
                ),
                trailing: IconButton(
                  icon: const Icon(Icons.delete_outline, color: Colors.red),
                  onPressed: () => FirebaseFirestore.instance.collection('feedbacks').doc(docId).delete(),
                ),
              ),
            );
          },
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 10, spreadRadius: 3)],
        ),
        child: Column(
          children: [
            Text("Add New Perfume", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: primary)),
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Center(child: Text(imgName != null ? (imgName!.length > 15 ? imgName!.substring(0, 15) : imgName!) : '')),
                ),
              ],
            ),
            const SizedBox(height: 15),
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text("Add Perfume", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ),
          ],
        ),
  ...
  Future<void> _pickImage() async {
    ...
    if (img == null) return;
    setState(() { isLoading = true; imgName = img.name; });
  ...
  Widget _inputField(String hint, TextEditingController controller, {int maxLines = 1}) {
    return Padding(
      ...
      child: TextField(
        ...
        maxLines: maxLines,
        decoration: InputDecoration(hintText: hint, filled: true, fillColor: bg, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))),
      ),
    );
  }
  ...
  Future<void> _addPerfume() async {
    if (nameController.text.isEmpty || priceController.text.isEmpty || imgUrl == null) return;
    ...
    nameController.clear(); priceController.clear(); descController.clear();
    setState(() { imgUrl = null; imgName = null; isLoading = false; });
  }

  Future<void> _signOut() async {
    await FirebaseAuth.instance.signOut();
    if (!mounted) return;
    Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => const RegisterLoginScreen()), (route) => false);
  }
}